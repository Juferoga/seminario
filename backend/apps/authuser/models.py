...
class CustomUserManager(UserManager):
  ...
  def create_user_by_type(user_type, email=None, password=None, **extra_fields):
    ...
    elif user_type == RoleChoices.STUDENT:
      extra_fields.setdefault("is_staff", False)
      extra_fields.setdefault("is_superuser", False)
      user = Customer.objects.create_student(email=email, **extra_fields)
    else:
  ...
  def create_student(self, email=None, password=None, **extra_fields):
    extra_fields.setdefault("is_staff", False)
    extra_fields.setdefault("is_superuser", False)
    extra_fields.setdefault("t_rol", 4)
    return self._create_user(email, password, **extra_fields)
...
class User(AbstractBaseUser, PermissionsMixin):
  ...
  t_rol = models.IntegerField(default=RoleChoices.STUDENT, choices=RoleChoices.choices)
